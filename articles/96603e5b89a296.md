...
`recordRoborazzi`ではなく`app:recordRoborazzi`や`cart:recordRoborazzi`といった、モジュール単位での実行を見たところ、app-moduleだけが失敗することが分かりました。

# Firebase関連の実装を消したり、Fakeに差し替えることで成功するようになった🎉

Firebaseが悪さをしていることは薄々気づいているので、Firebase関連の実装をコメントアウトしたり、mockに差し替えたりしました
...
ということでFirebaseがテストできないくしているのでモックなりフェイク実装なりで差し替える必要があることが分かりました。

# roborazziがPreviewを元にテストを自動生成しているので、差し替え不可能。。

さぁDI設定を差し替えればよいことは分かりましたが、次の問題が発生しました。
